...
export function Sidebar({ onClose, isCollapsed, onToggleCollapse }: { onClose?: () => void; isCollapsed?: boolean; onToggleCollapse?: () => void; }) {
    ...
    return (
        <aside className="w-full h-full border-r border-border/10 bg-background flex flex-col transition-all duration-300">
            <div className={`p-5 border-b border-border/10 flex items-center justify-between gap-3 h-16 ${isCollapsed ? 'justify-center p-0' : ''}`}>
            ...
            <nav className={`flex-1 py-5 space-y-0.5 overflow-y-auto overflow-x-hidden ${isCollapsed ? 'px-2' : 'px-3'}`}>
                {!isCollapsed && (
                ...
                {isAdmin && (
                    <>
                        ...
                        >
                            <LayoutGrid className={`h-4 w-4 flex-shrink-0 ${pathname === "/reports" ? "text-primary" : "text-muted-foreground/70"}`} strokeWidth={pathname === "/reports" ? 2 : 1.5} />
                        ...
                        >
                            <Users className={`h-4 w-4 flex-shrink-0 ${pathname === "/users" ? "text-primary" : "text-muted-foreground/70"}`} strokeWidth={pathname === "/users" ? 2 : 1.5} />
            ...
            <div className={`p-3 border-t border-border/10 mt-auto flex flex-col gap-2 ${isCollapsed ? 'items-center' : ''}`}>
                ...
                {onToggleCollapse && (
                    <button 
                        ...
                        className={`hidden md:flex items-center gap-3 py-2 rounded-lg text-[13px] text-muted-foreground hover:text-foreground hover:bg-muted/40 font-normal transition-all duration-200 ${isCollapsed ? 'justify-center px-0 w-full' : 'px-3'}`}
